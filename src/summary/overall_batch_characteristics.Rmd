---
title: "Overall batch characteristics"
author: "Almut Lütge"
date: "15 April 2020"
params:
  sce:
...
## Batch Characteristics in single cell RNAseq
...
sce_small_vec <- c(paste0(params$sce_norm, sce_name, "_sce.rds"))
...
general_info[c("pancreas", "cellbench", "hca"), "batch_origin"] <- "protocol"
...
# annotations plot (just for the manuscript plots)
h_cells <- HeatmapAnnotation(cells = anno_barplot(general_info$n_cells, 
                                                    gp = gpar(fill = col_type_vec),
                                                    height = unit(2.2, "cm"),
...
h_genes <- HeatmapAnnotation(genes = anno_barplot(general_info$n_genes, 
                                                    gp = gpar(fill = col_type_vec),
                                                    height = unit(2.2, "cm"),
                                                    axis_param = list(side = "right",
                                                                      gp = gpar(fontsize = 15),
...
h_data <- HeatmapAnnotation(dataset = anno_text(general_info$dataset,
                                                gp = gpar(fontsize = 18),
                                                rot = 45),
                            annotation_name_side = "left",
                            annotation_name_gp = gpar(fontsize = 18))                         
...
lgd_short = Legend(labels = names(col_type), 
                   legend_gp = gpar(fill = unlist(col_type)),
...
h_list2 <-  h_cells %v% h_genes %v% h_data

draw(h_list, annotation_legend_list = pd)
draw(h_list2,  annotation_legend_list = lgd_short)

saveRDS(h_list, paste0(out_path_fig, "_general_info.rds"))
saveRDS(pd, paste0(out_path_fig, "_general_info_lgd.rds"))
saveRDS(h_list2, paste0(out_path_fig, "_general_info_short.rds"))
saveRDS(lgd_short, paste0(out_path_fig, "_general_info_short_lgd.rds"))
...
batch_size <- lapply(sce_name, function(sce_nam){
  ...
  meta <- readRDS(meta_vec[sce_nam])
  d <- data.frame("PVE_batch" = sum$mean_var_batch,
                  "PVE_celltype" = sum$mean_var_celltype,
                  "mean_cms" = sum$mean_cms,
                  "DE_genes" = sum$mean_mean_n_de_genes)
}) %>% set_names(sce_name) %>% bind_rows() %>% set_rownames(sce_name)
...
# mean variance part
n_batches <- t(as.matrix(batch_size[,c("PVE_batch", "PVE_celltype")])) %>% 
...
mean_de_genes <- t(as.matrix(batch_size$DE_genes))
...
DE_genes <- t(as.matrix(batch_size$DE_genes))
colnames(DE_genes) <- batch_size$dataset
rownames(DE_genes) <- "DE_genes"

h_size2 <- Heatmap(DE_genes, 
          cluster_rows = FALSE, 
          cluster_columns = FALSE,
          name = "# genes",
          column_names_side = "bottom",
...
#just for figure plots
h_cor_fig <- Heatmap(cor_tab, 
          cluster_rows = FALSE, 
          cluster_columns = FALSE,
          name = "lfc correlation",
          column_names_side = "bottom",
