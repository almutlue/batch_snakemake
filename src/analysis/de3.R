...
doDE <- function(sce, lfc_cutoff = 0){
  res <- lapply(kids, function (k) {
    ...
    # #es_tmp <- es_tmp[k1, ]
    logfc <- predFC(dge, design, prior.count=0.07)
    y <- voom(dge, design, plot = T)
    f <- lmFit(y, design, method = "robust")
    f <- eBayes(f, trend = TRUE)
    tt <- lapply(names(cont), function(cc) {
        c1 <- gsub('-.*', '', cc)
        c2 <- gsub('.*-', '', cc)
        fc_pred <- logfc[,c1] - logfc[,c2]
        fc <- contrasts.fit(f, contrasts = cont[[cc]])
        tr <- treat(fc, lfc = lfc_cutoff)
        tt <- topTreat(tr, n = Inf)
        res_df(k, tt, ctype, cc, fc_pred)
