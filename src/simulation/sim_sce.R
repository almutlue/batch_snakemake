...
cd$sample_id <- as.factor(cd[, sam])
cd$cluster_id <- as.factor(cd[, celltype])
cd$batch_id <- as.factor(cd[, batch])

### Why does this NOT work for hca datset?? Sth with dplyr?
# cd <- dplyr::mutate(cd,
...
##########################################################################
####################### only relevant for hca dataset ####################
#adjust names with special characters and different namings (HCA dataset)
sce$batch_id <- gsub("\\(","", sce$batch_id)
sce$batch_id <- gsub("\\)","", sce$batch_id)
sce$batch_id <- gsub("CEL-Seq2","CELseq2", sce$batch_id)
sce$batch_id <- gsub("Drop-Seq","Dropseq", sce$batch_id)
sce$batch_id <- gsub("Smart-Seq2","SMARTseq2", sce$batch_id)
sce$batch_id <- gsub("\\-","", sce$batch_id)
if( data %in% 'out/edger/edgeR_csf_patient.rds'){
  sce$batch_id <- gsub("pat1","pat3", sce$batch_id)
}


sce$batch_id <- as.factor(sce$batch_id)

sce$sample_id <- gsub("\\(","", sce$sample_id)
sce$sample_id <- gsub("\\)","", sce$sample_id)
sce$sample_id <- gsub("CEL-Seq2","CELseq2", sce$sample_id)
sce$sample_id <- gsub("Drop-Seq","Dropseq", sce$sample_id)
sce$sample_id <- gsub("Smart-Seq2","SMARTseq2", sce$sample_id)
sce$sample_id <- gsub("\\-","", sce$sample_id)
sce$sample_id <- as.factor(sce$sample_id)

sce$cluster_id <- gsub("\\+", "", sce$cluster_id)
sce$cluster_id <- as.factor(sce$cluster_id)

names(rowData(sce)) <- gsub("\\(","", names(rowData(sce)))
names(rowData(sce)) <- gsub("\\)","", names(rowData(sce)))
names(rowData(sce)) <- gsub("\\+","", names(rowData(sce)))
names(rowData(sce)) <- gsub("C1HT-medium","C1HTmedium", names(rowData(sce)))
names(rowData(sce)) <- gsub("C1HT-small","C1HTsmall", names(rowData(sce)))
names(rowData(sce)) <- gsub("mcSCRB-Seq","mcSCRBSeq", names(rowData(sce)))
names(rowData(sce)) <- gsub("Quartz-Seq2","QuartzSeq2", names(rowData(sce)))
names(rowData(sce)) <- gsub("MARS-Seq","MARSSeq", names(rowData(sce)))
names(rowData(sce)) <- gsub("CEL-Seq2","CELseq2", names(rowData(sce)))
names(rowData(sce)) <- gsub("Drop-Seq","Dropseq", names(rowData(sce)))
names(rowData(sce)) <- gsub("Smart-Seq2","SMARTseq2", names(rowData(sce)))
...
simData <- function(x, n_genes = 500, n_cells = 300,
  ...
  a_vec <- grepl("A",colnames(sim_mean))
  b_vec <- grepl("B",colnames(sim_mean))
  colnames(sim_mean)[a_vec] <- paste0("A.",sids)
  colnames(sim_mean)[b_vec] <- paste0("B.",sids)
