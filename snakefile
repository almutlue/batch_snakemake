...
rule all:
	input:
	  ...
	  characterization(expand(config["out_vp_ct"] + "vp_ct_{sample}_sce.rds", sample = sample)),
	  ...
	  characterization(expand(config["out_type_var"] + "type_var_{sample}_sce.rds", sample = sample_sim)), 
	  ...
	  characterization(config["docs"] + "var_part_ct.html"),
	  characterization(config["docs"] + "batch_type_var_part.html"),
	  ...
	  simulation(expand(config["out_sim_vp_ct"] + "vp_ct_sim_{sample}_sce.rds", sample = sample_sim)),
	  ...
	  simulation(config["docs"] + "var_part_ct_sim.html"),
